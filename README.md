...
> 源码仓库提供图形控制器和 Kanata 配置；Release 压缩包提供已经打包好的 `SOCD_Controller.exe`。Kanata、Interception 等第三方二进制文件请从官方渠道下载。
...
如果不用 Release 压缩包，也可以手动把以下文件放在同一个目录：
...
然后打开 `SOCD_Controller.exe`，选择补偿模式和生效方向，点击“保存并启动”。
